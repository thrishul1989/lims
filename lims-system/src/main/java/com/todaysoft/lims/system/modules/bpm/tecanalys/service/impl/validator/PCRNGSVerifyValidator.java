...
public class PCRNGSVerifyValidator implements ITechnicalAnalyRecordValidator
{
    @Override
    public void validate(TechnicalAnalyRecordValidateContext context)
    {
        ...
        if (!"PCR-NGS".equalsIgnoreCase(verifyMethod))
        {
            return;
        }
        ...
        if (StringUtils.isEmpty(target.getData().getGeneSymbol()))
        {
            ...
            target.setMessage("PCR-NGS验证，突变基因字段为空");
            return;
        }
        ...
        {
            target.setValid(false);
            target.setMessage("PCR-NGS验证，染色体位置字段为空");
            return;
        }
        ...
        {
            target.setValid(false);
            target.setMessage("PCR-NGS验证，染色体字段为空");
            return;
        }
        ...
        {
            target.setValid(false);
            target.setMessage("PCR-NGS验证，位置1字段为空");
            return;
        }
