$(function()
{
	...
	// 禁用 启用
	$(".fa-circle-o").parent('a').css('background-color','rgb(26, 167, 96)');
	$(".fa-circle-o").parent('a').css('border-color','rgb(26, 167, 96)');
	
	$(".fa-ban").parent('a').css('background-color','#eb4f38');
	$(".fa-ban").parent('a').css('border-color','#eb4f38');
	
	// 警告
$(".fa-warning").parent('a').css('background-color','rgb(207, 208, 23)');
	
	$(".fa-warning").parent('a').css('border-color','rgb(207, 208, 23)');
...
function clearConditionExcludeData(formId){
	   $.each($("#"+formId).find('input,select').not('input[name="sequencingCode"]').not(":hidden"),function(){
		   $(this).val("");  
	   });
}
