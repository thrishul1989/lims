...
function downloadData(){
	...
	$.ajax({
		 type:"POST",
		 traditional: true,
		 url:"/testSheet/downloadFirstPCRData",
		 ...
		 if(list==''){
	          ...
	          $('#uploadDataTable tbody tr').each(function(i, v) {
	        	  ...
	        	  for(var m = 0; m < list.length; m++){
	        		  if(combineCode == list[m][2]){
	        			  $(this).find(".abnormalReason").empty();
	        			  
	        			  $(this).find(".result").text(list[m][3]);
		            	  $(this).find(".dispose").text(list[m][4]);
		            	  $(this).find(".remark").text(list[m][5]);
		            	  $(this).find(".abnormalReason").text(list[m][6]);
	        			  ...
	        			  if("成功" == list[m][3]){
							  ...
							  $(this).find(".abnormalReason").text("实验结果异常(要么成功要么失败)");
						  }
		            	  $(this).find(".disposeInput").val(list[m][4]);
		            	  $(this).find(".remarkInput").val(list[m][5]);
...
$(function()
{
    $('body').on('click', '#button_submit', function()
    {
    	$('#dataTable tbody tr').each(function(){
        	...
        	var pcrTestCode = $(this).find(".pcrTestCode").text();
        	var forwardPrimerLocationTemp = $(this).find(".forwardPrimerLocationTemp").text();
            if(""==taskId){
            	$('#qcPointTestCode').val(pcrTestCode);
            	$('#qcPointPrimerLocation').val(forwardPrimerLocationTemp);
            }
        });
        ...
        $('#uploadDataTable tbody tr').each(function(i, v){
        	...
        	var result = $(this).find(".result").text();
      	    var dispose = $(this).find(".dispose").text();
