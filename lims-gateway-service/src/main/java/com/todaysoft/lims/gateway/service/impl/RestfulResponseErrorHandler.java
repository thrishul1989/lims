...
public class RestfulResponseErrorHandler extends DefaultResponseErrorHandler
{
    ...
    public void handleError(ClientHttpResponse response) throws IOException
    {
        try
        {
            ...
            {
                case CLIENT_ERROR:
                    logger.error("Gateway client error, status " + status + ":" + response.getStatusText());
                    throw ServiceException.innerError();
                case SERVER_ERROR:
                    logger.error("Gateway server error, status " + status + ":" + response.getStatusText());
                    ...
                    throw e;
                default:
                    logger.error("Gateway error, status " + status + ":" + response.getStatusText());
                    throw ServiceException.innerError();
            }
        }
