...
public class DefaultInvoiceDelaySearcher implements ISearcher<DefaultInvoiceModel>
{
    ...
    public NamedQueryer toQuery()
    {
        ...
        hql.append("SELECT fit FROM FinanceInvoiceTask fit, Order o left join o.orderReduce r left join o.orderDelay od WHERE  fit.category = '1' AND o.id = fit.recordKey ");
    ...
    private void addFinanceInvoiceTaskFilter(StringBuffer hql, List<String> paramNames, List<Object> parameters)
    {
        ...
        if (StringUtils.isNotEmpty(isFullPay))
        {
            ...
            if ("1".equals(isFullPay))//应收款等于实收款
            {
                hql.append(" and (o.amount+o.subsidiarySampleAmount-o.discountAmount-o.incomingAmount -" + a + " ) = 0   )");
            }
            else if ("2".equals(isFullPay)) //应收款小于实收款 或者它已经同步到金税了
            {
                hql.append(" and ( ( od.status = '1' and ( o.amount+o.subsidiarySampleAmount-o.discountAmount-o.incomingAmount -" + a + " ) > 0  )) or (fit.synchroStatus = '1' ))");
            }
            else//应收款不等于实收款
            {
                hql.append(" and (o.amount+o.subsidiarySampleAmount-o.discountAmount-o.incomingAmount -" + a + " ) != 0   )");
