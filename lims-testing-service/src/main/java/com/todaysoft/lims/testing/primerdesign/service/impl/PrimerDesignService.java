package com.todaysoft.lims.testing.primerdesign.service.impl;
...
import com.todaysoft.lims.testing.primerdesign.context.PrimerDesignSubmitContext;
import com.todaysoft.lims.testing.primerdesign.context.PrimerDesignSubmitScheduleContext;
import com.todaysoft.lims.testing.primerdesign.context.PrimerDesignSubmitTaskContext;
import com.todaysoft.lims.testing.primerdesign.dao.PrimerDesignAssignableTaskSearcher;
import com.todaysoft.lims.testing.primerdesign.model.PrimerDesignAssignArgs;
import com.todaysoft.lims.testing.primerdesign.model.PrimerDesignAssignModel;
import com.todaysoft.lims.testing.primerdesign.model.PrimerDesignAssignRequest;
import com.todaysoft.lims.testing.primerdesign.model.PrimerDesignAssignTaskArgs;
...
@Service
public class PrimerDesignService implements IPrimerDesignService
{
    @Autowired
    ...
    @Override
    public List<PrimerDesignTask> getAssignableList(PrimerDesignAssignableTaskSearcher searcher, int flag)
    {
        ...
        List<PrimerDesignTask> tasks = new ArrayList<PrimerDesignTask>();
        
        for (TestingTask record : records)
        {
            tasks.add(wrap(record, flag));
        }
        tasks.sort(Comparator.comparing(PrimerDesignTask::getResubmitCount).reversed());
        tasks.sort(Comparator.comparing(PrimerDesignTask::getIfUrgent).reversed());
        return tasks;
    }
    
    @Override
    public PrimerDesignAssignModel getAssignableModel(PrimerDesignAssignArgs args)
    {
        PrimerDesignAssignModel model = new PrimerDesignAssignModel();
        ...
        PrimerDesignAssignableTaskSearcher searcher = new PrimerDesignAssignableTaskSearcher();
        searcher.setIncludeKeys(includeKeys);
        List<PrimerDesignTask> tasks = getAssignableList(searcher, 2);
    ...
    @Override
    @Transactional
    public void assign(PrimerDesignAssignRequest request, String token)
    {
        if (!CollectionUtils.isEmpty(request.getTasks()))
        {
            TestingTask testingTask;
            
            for (PrimerDesignAssignTaskArgs task : request.getTasks())
            {
    ...
    private TestingSheetCreateModel buildTestingSheetCreateModel(PrimerDesignAssignRequest request, String token)
    {
        UserMinimalModel loginer = smmadapter.getLoginUser(token);
        UserMinimalModel tester = smmadapter.getUserByID(request.getTesterId());
        TaskConfig taskConfig = bcmadapter.getTaskConfigBySemantic(TaskSemantic.PRIMER_DESIGN);
        ...
        model.setTaskSemantic(TaskSemantic.PRIMER_DESIGN);
    ...
    @Override
    public PrimerDesignSheetModel getTestingSheet(String id)
    {
        ...
        PrimerDesignSheetModel sheet = new PrimerDesignSheetModel();
        ...
        List<PrimerDesignTask> tasks = wrapForTesting(entity.getTestingSheetTaskList());
    ...
    @Override
    @Transactional
    public void submit(PrimerDesignSubmitRequest request, String token)
    {
        // 1、设置提交上下文数据
        PrimerDesignSubmitContext context = generateSubmitContext(request, token);
    ...
    private PrimerDesignSubmitContext generateSubmitContext(PrimerDesignSubmitRequest request, String token)
    {
        ...
        PrimerDesignSubmitContext context = new PrimerDesignSubmitContext();
    ...
    private void setContextForTestingSheetTask(TestingSheetTask sheetTask, PrimerDesignSubmitContext context)
    {
    ...
    private void doCreatePrimers(PrimerDesignSubmitContext context)
    {
        Set<PrimerDesignSubmitTaskContext> tasks = context.getSuccessTasks();
        
        String primerId;
        
        for (PrimerDesignSubmitTaskContext taskContext : tasks)
        {
    ...
    private void doUpdateTestingTasks(PrimerDesignSubmitContext context)
    {
        Set<PrimerDesignSubmitTaskContext> records = context.getRelatedTasks();
        
        TestingTask task;
        Date timestamp = new Date();
        TestingTaskResult result;
        
        for (PrimerDesignSubmitTaskContext record : records)
        {
    ...
    private void doUpdateSuccessTestingSchedules(PrimerDesignSubmitContext context)
    {
        Set<PrimerDesignSubmitScheduleContext> schedules = context.getSuccessSchedules();
        ...
        TaskConfig config = bcmadapter.getTaskConfigBySemantic(TaskSemantic.PCR_ONE);
        TaskConfig dnaConfig = bcmadapter.getTaskConfigBySemantic(TaskSemantic.DNA_EXTRACT);
        
        for (PrimerDesignSubmitScheduleContext scheduleContext : schedules)
        {
            ...
            if (null != sangerVerifyRecord.getPrimer() && null != sangerVerifyRecord.getDnaSample())
            {
                ...
                // 查询sanger验证表中的引物id跟DNAid，都不为空说明引物和DNA都OK，创建一次PCR任务
            ...
            {
                schedule.setActiveTask(dnaConfig.getName());
                baseDaoSupport.update(schedule);
            }
        }
    }
    
    private void doUpdateFailureTestingSchedules(PrimerDesignSubmitContext context)
    {
        //引物设计失败直接返回到技术分析，技术分析人员重新上传处理结果 （有可能换验证方式或不验证了）
        //失败的sanger验证 结束当前流程 并删除activeTask表流程相关的记录
        Set<PrimerDesignSubmitScheduleContext> schedules = context.getFailureSchedules();
        
        TestingSchedule schedule;
        
        TaskConfig config = bcmadapter.getTaskConfigBySemantic(TaskSemantic.PRIMER_DESIGN);
        ...
        for (PrimerDesignSubmitScheduleContext scheduleContext : schedules)
        {
            ...
            baseDaoSupport.update(schedule);
            //2.清除active表的流程相关的记录
            List<TestingScheduleActive> actives = testingScheduleService.getScheduleActives(schedule.getId(), TaskSemantic.PRIMER_DESIGN);
    ...
    private void doUpdatePrimerDesignSheet(PrimerDesignSubmitContext context)
    {
    ...
    private void doCompleteProcess(PrimerDesignSubmitContext context)
    {
        TestingSheet sheet = context.getSheetEntity();
        activitiService.submitTestingSheet(sheet.getId());
    }
    
    private List<PrimerDesignTask> wrapForTesting(List<TestingSheetTask> records)
    {
        if (CollectionUtils.isEmpty(records))
        {
            return Collections.emptyList();
        ...
        for (TestingSheetTask record : records)
        {
            ...
            tasks.add(wrap(testingTask, 3));
        }
        
        return tasks;
    }
    
    private PrimerDesignTaskVariables getTaskRunningVariables(String taskId)
    {
        String variables = testingTaskService.getVariables(taskId);
        
        if (StringUtils.isEmpty(variables))
        {
            return new PrimerDesignTaskVariables();
        }
        
        return JsonUtils.asObject(variables, PrimerDesignTaskVariables.class);
    }
    
    private PrimerDesignTask wrap(TestingTask entity, int flag)
    {
        PrimerDesignTask task = new PrimerDesignTask();
        ...
        PrimerDesignTaskVariables variables = getTaskRunningVariables(entity.getId());
        ...
        if (null != sangerVerifyRecord)
        {
            ...
            if (null != analyRecord)
            {
                ...
                String beginLocus = analyRecord.getBeginLocus();
                
                if (1 < flag)
                {
                    if (StringUtils.isNotEmpty(analyRecord.getChromosome()) && StringUtils.isNotEmpty(beginLocus))
                    {
                        PrimerDatumListRequest request = new PrimerDatumListRequest();
                        request.setChromosomeNumber(analyRecord.getChromosome());
                        request.setPcrPoint(Long.valueOf(beginLocus));
                        List<PrimerDatum> primerDatumList = bsmadapter.getprimerDatumList(request);
                        PrimerDatum primerDatum = Collections3.getFirst(primerDatumList);
                        if (null != primerDatum)
                        {
                            task.setPcrStartPoint(primerDatum.getPcrStartPoint());
                            task.setPcrEndPoint(primerDatum.getPcrEndPoint());
                            task.setForwardPrimerSequence(primerDatum.getForwardPrimerSequence());
                            task.setReversePrimerSequence(primerDatum.getReversePrimerSequence());
                        }
                    }
                }
    ...
    private void setPlannedFinishDate(TestingTask entity, PrimerDesignTask task)
    {
    ...
    private PrimerCreateRequest wrapPrimer(PrimerDesignSubmitTaskArgs args)
    {
    ...
    private String obtainPrimer(PrimerDesignSubmitTaskArgs args)
    {
        TestingMethod testingMethod = testingTaskService.getTestingMethodBySemantic(TestingMethod.SANGER);
