...
public class LongPcrAssignableTaskSearcher implements ISearcher<TestingTask>
{
    ...
    public NamedQueryer toQuery()
    {
        ...
        hql.append("FROM TestingTask t WHERE t.semantic = 'Long-PCR' AND t.status = :assignableStatus");
        ...
        {
            hql.append(
                " AND EXISTS (SELECT p.id FROM Product p WHERE sa.schedule.productId = p.id AND p.name LIKE :productName AND sa.schedule.verifyKey IS NULL)");
