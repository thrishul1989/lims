...
public class CompletedReportSearcher implements ISearcher<TestingReport>
{
    ...
    public NamedQueryer toQuery()
    {
        ...
        hql.append("SELECT tr FROM TestingReport tr, OrderProduct op WHERE tr.status = 2 AND op.order.id = tr.order.id AND op.product.id = tr.product.id");
        List<String> paramNames = new ArrayList<String>();
        List<Object> parameters = new ArrayList<Object>();
        addFilter(hql, paramNames, parameters);
        hql.append(" ORDER BY tr.resubmitCount DESC, op.reportTime DESC");
    ...
    private void addFilter(StringBuffer hql, List<String> paramNames, List<Object> parameters)
    {
        ...
        {
            hql.append(" AND op.reportStatus = :reportStatus)");
            paramNames.add("reportStatus");
            parameters.add(reportStatus);
        }
        if (StringUtils.isNotEmpty(reviewerName))
        {
            hql.append(" AND tr.reviewerName LIKE :reviewerName");
            paramNames.add("reviewerName");
            parameters.add("%" + reviewerName + "%");
        }
        
        if (StringUtils.isNotEmpty(productName) || StringUtils.isNotEmpty(technicalMan))
        {
