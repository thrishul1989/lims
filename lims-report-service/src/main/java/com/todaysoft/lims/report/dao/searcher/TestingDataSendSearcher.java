...
public class TestingDataSendSearcher implements ISearcher<TestingDataSend>
{
    ...
    public NamedQueryer toQuery()
    {
        ...
        hql.append("select s FROM TestingDataSend s ,Order o where 1=1 and s.orderId=o.id and o.testingStatus != 4  ");
    ...
    private void addFilter(StringBuffer hql, List<String> paramNames, List<Object> parameters)
    {
        ...
        if (null != statu)
        {
            ...
            if (0 == statu)
            {
                hql.append(" AND s.orderId in (select orderId from TestingDataSend m where m.statu = 0)");
            }
            else
            {
                hql.append(" AND s.orderId not in (select orderId from TestingDataSend m where m.statu = 0)");
            }
        }
    ...
    @Override
    public Class<TestingDataSend> getEntityClass()
    {
        // TODO Auto-generated method stub
        return TestingDataSend.class;
    }
}
